...
Expecting (X Y W H) or #(X Y W H). ")))))
    ...
    (alx:when-let ((after (cdr (assoc :after-initialize options))))
      ...
      (let* ((body   (rest after))
             (len    (length body))
             (pos1   (or (position :prelude  body) len))
             (pos2   (or (position :postlude body) len))
             (pos1*  (min pos1 pos2))
             (pos2*  (max pos1 pos2))
             (part1  (subseq body 0     pos1*))
             (part2  (subseq body pos1* pos2*))
             (part3  (subseq body pos2*)))
        (setf cached part1)
        (flet ((set-body (part)
                 (case (car part)
                   (:prelude  (setf cached-init  (cdr part)))
                   (:postlude (setf cached-after (cdr part))))))
          (when part2 (set-body part2))
          (when part3 (set-body part3)))))
