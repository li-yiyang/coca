...
(m1 air Sequoia 15.5 and m2 air Tahoe 26.2).
...
- [ ] [[https://github.com/gammasoft71/Examples_Cocoa/][Cocoa examples (github:gammasoft71/Examples_Cocoa)]],
  see [[file:examples/basic/readme.org][examples/basic/readme.org]]

  by the time it's finished, Coca could be thought as mostly
  finished and ready for real productition codes
- [ ] Deploy,
  expected to be =examples/deploy/readme.org=

  currently it's possible to =save-lisp-and-die= (with =coca-init=
  to reset ObjC environment), but may not able to deliver to
  end user
- [ ] Applications, expected to be under =application=

  those applications should only be experimental,
  not meant to be performance for actual productivity
  (just to proof that Coca could do so)

  - [ ] Lem Coca Backend,
    expected to be under =application/lem= as =coca/lem= subsystem
  - [ ] Nyxt Coca Backend,
    expected to be under =application/nyxt= as =coca/nyxt= subsystem
  - [ ] CLIM Coca Backend,
    +expected to be under =application/clim= as =coca/clim= subsystem+

    [2026-03-22] Please refer to [[https://codeberg.org/li-yiyang/McCLIM/src/branch/backend-coca/Backends/coca][McCLIM-Coca]] (WIP) for a minial ObjC binding
    and [[https://codeberg.org/McCLIM/McCLIM][McCLIM]] Cocoa backend implementation. You can see updates (screenshots,
    videos of newest progress) at [[https://mastodon.social/@lucky_magick][@lucky_magick Mastodon.social]]. 
...
+ =decode-xxx=:
  convert ObjC values into lisp value

  by default, =define-objc-enum= will automatically generate decoder
  for enum numbers
...
+ =xxxp= and =xxx-xxx-p=: boolean test slots should have accessor function
  with =p= (lisp style) testing names
